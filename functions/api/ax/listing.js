...
export async function onRequestPost(context) {
  ...
  if (!budget.ok) {
    ...
    logCall(context, { endpoint: 'listing', mode: 'demo', startedAt, reason: 'budget' })
    const demo = demoResult(input)
  ...
  const blocked = await checkRateLimitGroup(env, [
    { bucket: `ax:listing:${ip}`, maxHits: 10, notice: RATE_NOTICE.ip },
    { bucket: 'ax:listing:all', maxHits: 80, failOpen: false, notice: RATE_NOTICE.all },
  ])
  const rateNotice = blocked?.notice || null
  if (rateNotice) {
    const demo = demoResult(input)
    logCall(context, { endpoint: 'listing', mode: 'demo', startedAt, reason: 'rate_limit' })
    return json({ ...demo, ad_check: adCheckListing(demo, brand), brand_applied: Boolean(brand), notice: rateNotice })
  }
