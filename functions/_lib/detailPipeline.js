...
export const SECTION_BLUEPRINT = [
  ...
  // feature와 trust는 원래 둘 다 "원료·제조 방식"을 맡고 있었다. 역할이 겹치니
  // 두 섹션이 같은 사실을 되풀이했다(라이브 2회 모두 이 쌍에서 중복률 10~13%).
  // 그래서 feature는 "제품 자체의 사실", trust는 "구매를 망설이는 지점"으로 갈랐다.
  {
    key: 'feature',
    role: '제품 자체의 사실만 설명한다 — 원료·제조 방식·규격. 입력에 있는 정보만 쓰고, 없는 수치나 인증은 지어내지 않는다.',
  },
  {
    key: 'trust',
    role: '구매를 망설이게 하는 지점을 짚어 해소한다. 앞 섹션에서 말한 원료·제조 사실을 다시 설명하지 말고, 실제로 언제 어떻게 쓰게 되는지 장면을 그리거나 표기사항에서 직접 확인할 수 있다는 점으로 안심시킨다.',
  },
  {
    key: 'guide',
...
FAQ는 **위 본문이 이미 답하는 내용을 되묻지 마세요**. 원료·제조 방식·구성·보관법은 본문에서 다루므로, 그 밖에 구매자가 실제로 궁금해할 것(섭취·사용 대상, 알레르기나 주의사항 확인 방법, 배송·교환 등 구매 실무)을 물어보세요.`
}
