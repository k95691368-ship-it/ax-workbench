...
export function buildChannelRows(channelId, items) {
  ...
  for (const item of items || []) {
    ...
    const ctx = { title: fixed.title, product: item, listing, rule }
  ...
  return { headers: spec.columns.map((c) => c.header), rows, adjusted, blocked, label: spec.label, fileTag: spec.fileTag }
}
...
export function channelCsv(channelId, items) {
  const built = buildChannelRows(channelId, items)
