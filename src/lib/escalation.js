...
export const ESCALATION_RULES = [
  {
    ...
    words: [
      ...
      '탈이 났', '탈났', '쓰러',
      // 실제 리뷰에서 흔한 증상 호소인데 목록에 없어 통째로 지나갔다.
      // '두드러기'는 잡고 '가려움'은 못 잡으면, 같은 증상을 어떻게 적었느냐로 판정이 갈린다.
      // ('아프다'처럼 뜻이 넓은 말은 넣지 않는다 — "머리 아프게 고민했다" 류까지 잡는다)
      '가려움', '가렵', '따갑', '두통', '통증', '메스꺼', '울렁',
      // '중독'만 단독으로 두면 "중독성 있는 맛"·"중독적이에요" 같은 대표적 칭찬 표현이
      // 걸린다. 게다가 '중독'은 CRITICAL_WORDS라 부정 완화조차 건너뛰어, 5점 칭찬 리뷰에
      // 사과문(SAFE_REPLY)이 초안으로 달렸다. 실제 위험을 뜻하는 활용형만 남긴다.
      '중독 증상', '중독으로', '급성 중독',
    ],
  },
  {
  ...
  },
  {
    ...
    // '유리'·'썩'을 단독으로 두면 "유리병에 담아 보관", "가격 대비 유리한", "맛이 썩 별로"가
    // 모두 이물·변질 신고로 올라갔다. 이물을 특정하는 구와 변질 활용형으로만 좁힌다.
    words: [
      '이물', '벌레', '곰팡이', '머리카락', '상한 것', '상했', '부패',
      '유리 조각', '유리조각', '유리가 들어', '유리 파편',
      '썩은', '썩어', '썩는', '썩었',
    ],
  },
]
...
export const CRITICAL_WORDS = new Set([
  // '중독'은 빼고 '식중독'만 둔다 — 단독 '중독'은 "중독성 있는 맛" 같은 칭찬에 걸리는데
  // critical이라 부정 완화도 건너뛰어 어떤 문맥으로도 구제되지 않았다.
  '응급실', '입원', '쓰러', '식중독', '사망',
